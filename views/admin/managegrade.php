...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['deleteGrade'])) {
        ...
        $query = "DELETE FROM grade_class WHERE grade_id = :grade_id";
        $stmt = $pdo->prepare($query);
        $stmt->execute([':grade_id' => $grade_id]);
        ...
        <div id="gradeForm" class="form-container" style="display: none;">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="grade" name="grade" placeholder="Enter Grade Name" required>
                </div>

                <!-- Grade Image -->
                <div class="form-group">
                    <label for="image">Grade Image</label>
                    <input type="file" id="image" name="image" accept="image/*">
                </div>

                <!-- Save Button -->
                <button type="submit" name="saveGrade" class="save-grade-btn">Save Grade</button>
            </form>
        </div>
    </div>
</div>
