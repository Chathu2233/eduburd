...
try {
    // Fetch the student's student_id using their user_id
    ...
    $requestSent = !empty($existingRequest); // Check if a request already exists
} catch (PDOException $e) {
    die("Error fetching data: " . $e->getMessage());
}

// Check if a request already exists and fetch its status
try {
    $stmtCheck = $pdo->prepare("
        SELECT status 
        FROM tutor_student_request 
                ...
                <img src="<?= ROOT ?>/<?= htmlspecialchars($tutor['profile_photo']) ?>" alt="Profile Image"style="width: 150px; height: 150px; object-fit: cover; border-radius: 50%; border: 2px solid #ddd; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin: 10px auto; display: block;">                    
                 ...
                 <h2><?= htmlspecialchars($tutor_name) ?></h2>
                    ...
                    <button 
                        id="request-btn" 
                        class="request-btn" 
                        onclick="sendRequest(<?= htmlspecialchars($tutor_id) ?>, <?= htmlspecialchars($student_id) ?>)" 
